<%-- 
    ...
    Created on : Jun 13, 2016, 6:53:23 AM
       ...
       List list = null; 
        ...
        try{
           customerid = session.getAttribute("customerid").toString();
            ...
            list = inl.viewOrderByCustomerID(customerid);
        }catch(Exception ex){
...
<table width="80%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td height="17" class="tableheader" >..::All Orders ..</td>
    </tr>
</table>
<table width="80%" border="0" cellspacing="0" cellpadding="0" id="results"  class="table table-striped table-hover table-bordered" id="sample_editable_1">
  ...
  <tr>
    ...
    <%
      ...
      <%
                ...
                while(it.hasNext()){
                        h++;
                        Order1bean sh = (Order1bean)it.next();
			     if(count%2==0){
				 style = "even";
				 }else{
				 style = "odd";
  ...
  %>
  <tr class="<%=style%>">
      <td height="28" class="tab_lebel"><%=sh.getOrdernumber()%></td>
    <td><%=sh.getProductcode()%></td>
    <td><%=sh.getDateoforder()%></td>
    <td><%=sh.getStatus()%></td> 
    <td><%=sh.getAmount()%></td>
    <td>
        <%=sh.getShippingmethod()%>
       </td>
    </tr>
   <%
	count++;
